const SPREADSHEET_ID = "101sBGIKK8AWMvwXt-AcffUyPuJD-aaAnk1Y2JN-XU3k";
const PROGRAM_CODE = "august_2026_3d_printing";
const PROGRAM_NAME = "3D Printing";
const STRIPE_PRODUCT_NAME = "Kids 3D Printing - 6 Sessions";
const STRIPE_METADATA_PROGRAM = "kids_3d_printing";
...
function doGet() {
  return jsonResponse_({
    success: true,
    message: "3D Printing registration endpoint is running.",
...
function setupPeriodWorkbook() {
  ...
  if (products.getLastRow() === 1) {
    products.getRange(2, 1, 1, PRODUCT_HEADERS.length).setValues([[
      PROGRAM_CODE,
      "KIDS_3D_PRINTING_6_SESSION",
      "Kids 3D Printing - 6 Sessions",
      "2026-08-17",
      "",
      "",
      "",
  ...
  return "3D Printing workbook is ready for version " + SCRIPT_VERSION + ".";
}
...
function normalizeSelectedItemCodes_(value) {
  if (!Array.isArray(value) || value.length !== 1) {
    throw clientError_("Choose the six-session 3D Printing class before payment.");
  }
...
function validateReturnUrl_(value, kind) {
  ...
  if (!/^https:\/\/(?:www\.)?sparkpreneurs\.ca\/3d-printing\/?(?:\?[^#]*)?(?:#[\s\S]*)?$/.test(url)) {
...
function assertProgramCode_(value) {
  if (String(value || "").trim() !== PROGRAM_CODE) {
    throw clientError_("This registration is not for 3D Printing.");
  }
}
