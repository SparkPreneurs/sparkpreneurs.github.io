const SPREADSHEET_ID = "1aCcU4TIY8AB_KzYUkbTmU61BEgVv1VM9Vg3AQFxZIo0";
const PROGRAM_CODE = "fall_2026_mixed_media_art";
const PROGRAM_NAME = "Mixed Media Art";
const STRIPE_PRODUCT_NAME = "Kids Mixed Media Art - 8 Sessions";
const STRIPE_METADATA_PROGRAM = "kids_mixed_media";
const PROGRAM_CATEGORY = "kids_youth";
const PROGRAM_TYPE = "class";
const PROGRAM_SESSIONS = "8";
const PROGRAM_DAY = "Wednesday";
const PROGRAM_START_TIME = "17:00";
const PROGRAM_END_TIME = "18:30";
const PROGRAM_ROUTE = "/mixed-media/index.html";
const REGULAR_PRICE_CENTS = 20000;
const EARLY_BIRD_PRICE_CENTS = 17000;
const EARLY_BIRD_DISCOUNT_PERCENT = 15;
const CURRENCY = "cad";
const SCRIPT_VERSION = "2026-09-01-mixed-media-2";
...
function doGet() {
  return jsonResponse_({
    success: true,
    message: "Mixed Media Art registration endpoint is running.",
...
function setupPeriodWorkbook() {
  ...
  if (products.getLastRow() === 1) {
    products.getRange(2, 1, 1, PRODUCT_HEADERS.length).setValues([[
      PROGRAM_CODE,
      "KIDS_MIXED_MEDIA_ART_8_SESSION",
      "Kids Mixed Media Art - 8 Sessions",
      "",
      "",
      PROGRAM_START_TIME,
      PROGRAM_END_TIME,
  ...
  return "Mixed Media Art workbook is ready for version " + SCRIPT_VERSION + ".";
}
...
function createCheckoutSession_(data) {
  ...
  try {
    checkoutSession = stripeRequest_("post", "/checkout/sessions", {
      ...
      "metadata[schedule]": PROGRAM_DAY.toLowerCase(),
      "metadata[time]": PROGRAM_START_TIME + "-" + PROGRAM_END_TIME,
      "metadata[pricing_type]": "early_bird",
      "metadata[regular_price]": String(REGULAR_PRICE_CENTS),
      "metadata[checkout_price]": String(pricing.totalCents),
      "metadata[amount]": String(pricing.totalCents),
...
function normalizeSelectedItemCodes_(value) {
  if (!Array.isArray(value) || value.length !== 1) {
    throw clientError_("Choose the eight-session Mixed Media Art class before payment.");
  }
...
function validateReturnUrl_(value, kind) {
  ...
  if (!/^https:\/\/(?:www\.)?sparkpreneurs\.ca\/mixed-media\/?(?:\?[^#]*)?(?:#[\s\S]*)?$/.test(url)) {
...
function assertProgramCode_(value) {
  if (String(value || "").trim() !== PROGRAM_CODE) {
    throw clientError_("This registration is not for Mixed Media Art.");
  }
}
...
function logError_(err) {
  console.log("Mixed Media Art request failed: " + String(err && err.message ? err.message : err));
}
...
function sendEnrollmentNotificationSafely_(registration, programName, paidAt) {
  try {
    ...
    console.log("Mixed Media Art enrollment email failed: " + String(error && error.message ? error.message : error));
  }
}
